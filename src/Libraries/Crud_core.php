...
class Crud_core
{
    protected $schema, // table schema
        $base, //prefix uri or parrent controller.
        $main_segment,
        $edit_segment,
        $form_id,
        $redirects = true,
        $add_item = true,
        $add_card_header = true,
        $edit_card_header = true,
        $table_card_header = true,
        $search = true,
    ...
    function __construct($params, RequestInterface $request)
    {
        ...
        $this->base = $params['base'] ?? null;
        $this->main_segment = $params['main_segment'] ?? null;
        $this->edit_segment = $params['edit_segment'] ?? 'edit';


        $this->table_title = $params['table_title'] ?? 'All items';
        $this->form_submit = $params['form_submit'] ?? 'Submit';
        $this->form_title_update = $params['form_title_update'] ?? 'Update Item';
        $this->form_submit_update = $params['form_submit_update'] ?? 'Update';
        $this->form_title_add = $params['form_title_add'] ?? 'Create Item';
        $this->form_id = $params['form_id'] ?? '1';
        $this->add_item = $params['add_item'] ?? true;
        $this->add_card_header = $params['add_card_header'] ?? true;
        $this->edit_card_header = $params['edit_card_header'] ?? true;
        $this->table_card_header = $params['table_card_header'] ?? true;
        $this->search = $params['search'] ?? true;
        //Field options
    ...
    function view($page_number, $per_page, $columns, $where = null, $order = null)
    {

        //$root_url = $this->base . '/' . $this->table;

        $total_rows = $this->model->countTotalRows($this->table, $where, $this->request, $this->schema, $this->fields, $columns);
    ...
    protected function field_radio($field_type, $label, $field_params, $values)
    {
        $required = (isset($field_params['required']) && $field_params['required'] ? ' required ' : '');
        ...
        foreach ($values as $value) {
            ...
            $input .= '<label class="form-check-label" for="' . $rid . '">';
            $input .= '<input type="radio" class="form-check-input" id="' . $rid . '" name="' . $field_type . '" value="' . $value->{$pk} . '" ' . set_radio($field_type, $value->{$pk}, (isset($this->current_values->{$field_type}) && $this->current_values->{$field_type} == $value->{$pk} ? TRUE : FALSE)) . ' >' . $display_val;
            $input .= '</label>';
            $input .= '</div>';
        }
        return $this->input_wrapper($field_type, $label, $input, $required);
    ...
    protected function field_file($field_type, $label, $field_params)
    {
        ...
        $input = '<div class="custom-file">
                      ...
                      id="' . $field_type . '">
                      <label class="custom-file-label" for="customFile">' . label('app.string.ChooseFile') . '</label>
                    </div>';
    ...
    protected function items_table($columns = null, $items, $pagination)
    {
        ...
        foreach ($items as $item) {
            ...
            $table .= '<td class="text-center"><a href="' . $editUrl . '" class="btn btn-success btn-sm">Edit</a></td>';
            $table .= '</tr>';
        }
    ...
    public function getValues($field = false)
    {
        ...
        return $this->current_values->{$field} ?? false;
    }
    function getBaseUrl()
    {
